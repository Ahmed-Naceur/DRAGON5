SUBROUTINE VALU6B(IELEM,NUN,LXH,NBLOS,ISPLH,X,Y,SIDE,EVECT,ISS,KFLX,IXLG, &
& IYLG,MATXYZ,VALUE)
  !
  !-----------------------------------------------------------------------
  !
  !Purpose:
  ! Interpolate the flux distribution for DUAL method in 2D hexagonal.
  !
  !Copyright:
  ...
  ! NBLOS   number of lozenges per direction in 2D with mesh-splitting.
  ...
  !----
  INTEGER IELEM,NUN,LXH,NBLOS,ISPLH,IXLG,IYLG,ISS(3,NBLOS),KFLX(3,NBLOS), &
  & MATXYZ(IXLG,IYLG)
  REAL X(IXLG),Y(IYLG),SIDE,EVECT(NUN),VALUE(IXLG,IYLG)
  !----
  !  LOCAL VARIABLES
  !----
  INTEGER I,J,L,ISU,I1,I2,IE,IC,ICMIN,ICMAX,NBC,ISPX,ISPY,IND,IUN,IEL
  LOGICAL FVALUE
  DOUBLE PRECISION COEF(2,5),FLX(5),FLY(5),U,V,AXY(2),BXY(2),CXY(2), &
     & CENTER(2),SUMMIT(2,6),NRING,ANGLE,RADIUS,SIDEH,XYC(2),XYR(2)
  CHARACTER HSMG*131
  ...
  IF(INT(NRING) /= NRING) THEN
    CALL XABORT('VALU6B: INVALID NUMBER OF HEXAGONS IN THE PLANE.')
  ENDIF
  SIDEH=SIDE*ISPLH
  ...
  DO IC=1,LXH
    DO ISPY=1,ISPLH
      DO ISPX=1,ISPLH
        IEL=((IC-1)*ISPLH+ISPY-1)*ISPLH+ISPX
        MATXYZ(I,J)=ISS(1,IEL)
        DO ISU=1,3
          ...
          FVALUE=VALB_CONTAINS(IP%AXY_S,IP%BXY_S,IP%CXY_S,IP%DXY_S,XYC(1),XYC(2))
          IF(FVALUE) THEN
            ! lozenge rotation
            ...
            DO I2=1,IELEM
              DO I1=1,IELEM
                ...
                IUN=KFLX(ISU,IEL)+(I1-1)*IELEM+I2-1
                VALUE(I,J)=VALUE(I,J)+EVECT(IUN)*REAL(FLX(I1)*FLY(I2))
              ENDDO
            ENDDO
            GO TO 100
          ENDIF
  ...
  DEALLOCATE(LOZ)
  RETURN
  200 FORMAT(8HVALU6B: ,A1,1H=,1P,E15.8,31H IS OUTSIDE (-1/2,1/2) SUPPORT.)
  !
  CONTAINS
    FUNCTION VALB_CONTAINS(AXY, BXY, CXY, DXY, X, Y) RESULT(FVALUE)
    ...
    END FUNCTION VALB_CONTAINS
END SUBROUTINE VALU6B
