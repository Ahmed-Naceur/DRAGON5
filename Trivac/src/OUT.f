...
*----
      ...
      INTEGER IGP(NSTATE)
      TYPE(C_PTR) IPMAC1,IPMAC2,IPFLUX,IPVAL,IPTRK,IPGEOM
...
*----
      CALL LCMLEN(KENTRY(2),'LINK.TRACK',ILONG,ITYLCM)
      IF(ILONG.EQ.0) CALL XABORT('OUT: LINK.TRACK RECORD MISSING.')
   ...
   20 CALL LCMLEN(KENTRY(2),'LINK.MACRO',ILONG,ITYLCM)
      IF(ILONG.EQ.0) CALL XABORT('OUT: LINK.MACRO RECORD MISSING.')
      CALL LCMGTC(KENTRY(2),'LINK.MACRO',12,TEXT12)
      DO 50 I=1,NENTRY
      IF(HENTRY(I).EQ.TEXT12) THEN
         IPMAC1=KENTRY(I)
...
*----
      IF(C_ASSOCIATED(IPFLUX)) THEN
        CALL OUTDRV(IPGEOM,IPMAC1,IPFLUX,IPMAC2,MAXNEL,NBMIX,NL,NBFIS,
     1  NGRP,NEL,NUN,NALBP,HTRACK,IELEM,ICOL,MAT,VOL,IDL,TITLE,IBFP)
      ELSE IF(C_ASSOCIATED(IPVAL)) THEN
        CALL OUTVOX(IPGEOM,IPMAC1,IPVAL,IPMAC2,NBMIX,NL,NBFIS,NGRP,
     1  NALBP,TITLE)
      ENDIF
*----
