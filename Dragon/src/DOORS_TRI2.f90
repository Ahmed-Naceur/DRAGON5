SUBROUTINE DOORS_TRI2(IPTRK,NANIS,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO,FUNKNO)
  ...
  IF((ITYPE.EQ.2).OR.(ITYPE.EQ.5).OR.(ITYPE.EQ.7)) THEN
    ...
    CALL DOORS_TRI2GSO(IPTRK,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO,FUNKNO)
  ELSE IF((ITYPE.EQ.8).OR.(ITYPE.EQ.9))  THEN
    ! Hexagonal 2D or 3D geometry
    CALL DOORS_TRI2GSR2(IPTRK,NREG,NMAT,NUN,MATCOD,SIGG,SUNKNO,FUNKNO)
  ELSE
    CALL XABORT('DOORS_TRI2: GEOMETRY TYPE NOT AVAILABLE.')
  ENDIF
  RETURN
CONTAINS
  SUBROUTINE DOORS_TRI2GSO(IPTRK,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO,FUNKNO)
    !
    !-----------------------------------------------------------------------
    !
    !Purpose:
    ...
    IF((ITYPE.NE.2).AND.(ITYPE.NE.5).AND.(ITYPE.NE.7)) THEN
      CALL XABORT('DOORS_TRI2GSO: INVALID CARTESIAN GEOMETRY.')
    ELSE IF((IELEM.LT.0).OR.(ICOL.GT.3)) THEN
      CALL XABORT('DOORS_TRI2GSO: RAVIART-THOMAS METHOD EXPECTED.')
    ELSE IF(ISCAT.GT.1) THEN
      WRITE(HSMG,'(57HDOORS_TRI2GSO: MACRO-CALCULATION WITH ANISOTROPIC SCATTER, &
      & 66HING CURRENTLY NOT IMPLEMENTED; USE SCAT 1 KEYWORD IN TRIVAT: DATA.)')
      CALL XABORT(HSMG)
    ELSE IF(LX*LY*LZ.NE.NREG) THEN
      CALL XABORT('DOORS_TRI2GSO: INVALID NREG.')
    ENDIF
    ...
    DEALLOCATE(KN)
    RETURN
  END SUBROUTINE DOORS_TRI2GSO
  !
  SUBROUTINE DOORS_TRI2GSR2(IPTRK,NREG,NMAT,NUN,MATCOD,SIGG,SUNKNO,FUNKNO)
    !
    !-----------------------------------------------------------------------
    !
    !Purpose:
    ...
    IF((ITYPE.NE.8).AND.(ITYPE.NE.9)) THEN
      CALL XABORT('DOORS_TRI2GSR: INVALID HEXAGONAL GEOMETRY.')
    ELSE IF((IELEM.LT.0).OR.(ICOL.GT.3)) THEN
      CALL XABORT('DOORS_TRI2GSR: RAVIART-THOMAS METHOD EXPECTED.')
    ELSE IF(ISCAT.GT.1) THEN
      WRITE(HSMG,'(57HDOORS_TRI2GSR: MACRO-CALCULATION WITH ANISOTROPIC SCATTER, &
      & 66HING CURRENTLY NOT IMPLEMENTED; USE SCAT 1 KEYWORD IN TRIVAT: DATA.)')
      CALL XABORT(HSMG)
    ELSE IF(3*NBLOS.NE.NREG) THEN
      CALL XABORT('DOORS_TRI2GSR: INVALID NREG.')
    ENDIF
    CALL LCMLEN(IPTRK,'KN',MAXKN,ITYLCM)
