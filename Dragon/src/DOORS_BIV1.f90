SUBROUTINE DOORS_BIV1(IPTRK,NANIS,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
  ...
  IF((ITYPE.EQ.2).OR.(ITYPE.EQ.5)) THEN
    ...
    IF((IELEM.GT.0).AND.(ICOL.LE.3)) THEN
      ...
      CALL DOORS_BIV1GSO(IPTRK,NANIS,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
    ELSE IF((IELEM.LT.0).AND.(NLF.EQ.0)) THEN
      ! Lagrange / diffusion
      CALL DOORS_BIV1FSO1(IPTRK,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
    ELSE
  ...
  ELSE IF(ITYPE.EQ.8) THEN
    ...
    ELSE IF((IELEM.LT.0).AND.(NLF.EQ.0)) THEN
      ! Lagrange / diffusion
      CALL DOORS_BIV1FSH1(IPTRK,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
    ELSE
  ...
  RETURN
CONTAINS
  SUBROUTINE DOORS_BIV1FSO1(IPTRK,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
    !
    !-----------------------------------------------------------------------
    !
    !Purpose:
    ...
    RETURN
  END SUBROUTINE DOORS_BIV1FSO1
  !
  SUBROUTINE DOORS_BIV1GSO(IPTRK,NANIS,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
    !
    !-----------------------------------------------------------------------
    !
    !Purpose:
    ...
    ISCAT=JPAR(16)
    LHEX=(ITYPE.EQ.8)
    IF((IELEM.LT.0).OR.(ICOL.GT.3)) CALL XABORT('DOORS_BIV1GSO: RAVIA' &
    ...
    DEALLOCATE(KN)
    RETURN
  END SUBROUTINE DOORS_BIV1GSO
  !
  SUBROUTINE DOORS_BIV1FSH1(IPTRK,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
    !
    !-----------------------------------------------------------------------
    !
    !Purpose:
