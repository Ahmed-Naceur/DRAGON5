SUBROUTINE DOORS_TRI1(IPTRK,NANIS,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
  !
  !-----------------------------------------------------------------------
  !
  !Purpose:
  ! Compute the source for the solution of diffusion or PN equations.
  ! Use a TRIVAC tracking with a flat flux approximation
  !
  !Copyright:
  ...
  IF(JPAR(1).NE.NREG) CALL XABORT('DOORS_TRI1: INCONSISTENT NREG.')
  IF(JPAR(2).NE.NUN) CALL XABORT('DOORS_TRI1: INCONSISTENT NUN.')
  IF(NANIS.NE.0) CALL XABORT('DOORS_TRI1: SPN NOT IMPLEMENTED.')
  ITYPE=JPAR(6)
  IF((ITYPE.EQ.2).OR.(ITYPE.EQ.5).OR.(ITYPE.EQ.7)) THEN
    ! Cartesian 1D, 2D or 3D geometry
    CALL DOORS_TRI1GSO(IPTRK,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
  ELSE IF((ITYPE.EQ.8).OR.(ITYPE.EQ.9))  THEN
    ! Hexagonal 2D or 3D geometry
    CALL DOORS_TRI1GSR1(IPTRK,NREG,NMAT,NUN,MATCOD,SIGG,SUNKNO)
  ELSE
    CALL XABORT('DOORS_TRI1: GEOMETRY TYPE NOT AVAILABLE.')
  ENDIF
  RETURN
CONTAINS
  SUBROUTINE DOORS_TRI1GSO(IPTRK,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
    !
    !-----------------------------------------------------------------------
    !
    !Purpose:
    ...
    IF((ITYPE.NE.2).AND.(ITYPE.NE.5).AND.(ITYPE.NE.7)) THEN
      CALL XABORT('DOORS_TRI1GSO: INVALID CARTESIAN GEOMETRY.')
    ELSE IF((IELEM.LT.0).OR.(ICOL.GT.3)) THEN
      CALL XABORT('DOORS_TRI1GSO: RAVIART-THOMAS METHOD EXPECTED.')
    ELSE IF(ISCAT.GT.1) THEN
      WRITE(HSMG,'(57HDOORS_TRI1GSO: MACRO-CALCULATION WITH ANISOTROPIC SCATTER, &
      & 66HING CURRENTLY NOT IMPLEMENTED; USE SCAT 1 KEYWORD IN TRIVAT: DATA.)')
      CALL XABORT(HSMG)
    ELSE IF(LX*LY*LZ.NE.NREG) THEN
      CALL XABORT('DOORS_TRI1GSO: INVALID NREG.')
    ENDIF
    ...
    DEALLOCATE(KN)
    RETURN
  END SUBROUTINE DOORS_TRI1GSO
  !
  SUBROUTINE DOORS_TRI1GSR1(IPTRK,NREG,NMAT,NUN,MATCOD,SIGG,SUNKNO)
    !
    !-----------------------------------------------------------------------
    !
    !Purpose:
    ...
    IF((ITYPE.NE.8).AND.(ITYPE.NE.9)) THEN
      CALL XABORT('DOORS_TRI1GSR: INVALID HEXAGONAL GEOMETRY.')
    ELSE IF((IELEM.LT.0).OR.(ICOL.GT.3)) THEN
      CALL XABORT('DOORS_TRI1GSR: RAVIART-THOMAS METHOD EXPECTED.')
    ELSE IF(ISCAT.GT.1) THEN
      WRITE(HSMG,'(57HDOORS_TRI1GSR: MACRO-CALCULATION WITH ANISOTROPIC SCATTER, &
      & 66HING CURRENTLY NOT IMPLEMENTED; USE SCAT 1 KEYWORD IN TRIVAT: DATA.)')
      CALL XABORT(HSMG)
    ELSE IF(3*NBLOS.NE.NREG) THEN
      CALL XABORT('DOORS_TRI1GSR: INVALID NREG.')
    ENDIF
    CALL LCMLEN(IPTRK,'KN',MAXKN,ITYLCM)
  ...
  END SUBROUTINE DOORS_TRI1GSR1
END SUBROUTINE DOORS_TRI1
