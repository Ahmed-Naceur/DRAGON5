...
*----
      ...
      IF(CXDOOR.EQ.'MCCG') THEN
         ...
         IF(ICREB.GT.0) THEN
            ...
            NNN=ILONG-ICREB
            ALLOCATE(KEYCUR(NSOUT),V(ILONG),MATALB(ILONG),ALBEDO(NBCDA))
...
*
  ...
  230    CONTINUE
      ...
      IF(LREBAL.AND.(ITYPEC.NE.5)) THEN
         CALL FLUBAL(IPMACR,NGRP,ILEAK,NBCDA,NMAT,NREG,ICREB,NUNKNO,
     1   NANIS,MATCOD,VOL,KEYFLX(1,1,1),XSTRC,XSDIA,XCSOU,IGDEB,B2,
     2   NMERG,IMERG,DIFHET,KEYCUR,MATALB(NNN+1),ALBEDO,V(NNN+1),
     3   FLUX(1,1,7))
      ENDIF
*----
...
*----
      IF(CXDOOR.EQ.'MCCG') THEN
         IF(ICREB.GT.0) DEALLOCATE(ALBEDO,MATALB,V,KEYCUR)
      ENDIF
*----
*  SAVE THE SOLUTION
*----
