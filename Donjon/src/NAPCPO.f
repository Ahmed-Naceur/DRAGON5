...
*----
     ...
     1  COMMEN(MAXLIN)*80,PARKEL(MAXPAR)*12,VALH(MAXPAR)*12
      TYPE(C_PTR) JPCPO,KPCPO,LPCPO,JPMIC,JPFLU,KPFLU,MPFLU
...
C     project flux 
      ...
      CALL LCMLEN(IPFLU,'FLUX',LENGTH,ITYLCM)
      IF(LENGTH.GT.0) THEN
        MPFLU=LCMGID(IPFLU,'FLUX')
      ELSE
        CALL LCMLEN(IPFLU,'MODE',LENGTH,ITYLCM)
        IF(LENGTH.GT.0) THEN
          JPFLU=LCMGID(IPFLU,'MODE')
          KPFLU=LCMGIL(JPFLU,1)
          MPFLU=LCMGID(KPFLU,'FLUX')
        ELSE
          CALL LCMLIB(IPFLU)
          CALL XABORT('NAPCPO: UNABLE TO RECOVER A DIRECT FLUX.')
        ENDIF
      ENDIF
      DO IG=1,NG
        CALL LCMGDL(MPFLU,IG,FLXD)
        DO IP=1,NXP
          DO JP=1,NYP
            IREGP=IP+(JP-1)*NXP
